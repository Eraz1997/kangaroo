use crate::mock::create_mock_app;
use axum::{
    body::Body,
...
async fn send_test_request(uri: &str) -> TestResponse {
...
async fn not_found_error_gives_404() {
    let response = send_test_request("/404").await;
...
async fn parametrised_path_succeeds() {
    for parameter in &["example-1", "example-2", "example-3"] {
        let uri = format!("/{}", parameter);
        let response = send_test_request(&uri).await;
...
async fn custom_succeeds() {
    let response = send_test_request("/custom").await;
...
async fn absent_document_gives_internal_error() {
    let response = send_test_request("/absent-document").await;
...
async fn invalid_document_gives_internal_error() {
    let response = send_test_request("/invalid-document").await;
...
async fn deleted_resources_gives_status_code_specific_page() {
    let response = send_test_request("/deleted").await;
...
async fn custom_deleted_resources_gives_custom_status_code_specific_page() {
    let response = send_test_request("/deleted-custom").await;
